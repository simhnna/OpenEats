...
class recipeViewsTestCase(WebTest):
    ...
    def test_private(self):
        ...
        resp = self.app.get(reverse('recipe_show', kwargs={'slug': recipe.slug}),
    ...
    def test_rate(self):
        ...
        # add a vote
        resp = self.app.get(reverse('recipe_rate', kwargs={'object_id': 1, 'score': 4}),
                            user='testUser')
        self.assertEqual(resp.status_code, 200)
    ...
    def test_store(self):
        ...
        resp = self.app.post(reverse('recipe_store', kwargs={'object_id': 1}),
                             {'object_id': 1}, user='testUser')
        self.assertEqual(resp.status_code, 200)
    ...
    def test_unstore(self):
        ...
        resp = self.app.post(reverse('recipe_unstore'),
                             {'recipe_id': 10000}, user='testUser', status=404)
        self.assertEqual(resp.status_code, 404)
    ...
    def test_report(self):
        ...
        resp = self.app.post(reverse('recipe_report', kwargs={'slug': recipe.slug}),
                             user='testUser2')
        self.assertEqual(resp.status_code, 200)
        ...
        resp = self.app.post(reverse('recipe_report', kwargs={'slug': 'bad-recipe'}),
                             user='testUser2', status=404)
        self.assertEqual(resp.status_code, 404)
    ...
    def test_edit(self):
        ...
        resp = self.app.get(reverse('recipe_edit', kwargs={'user': 'admin', 'slug': recipe.slug}),
                            user='admin')
        self.assertEqual(resp.status_code, 200)
        ...
        self.assertEqual(resp.request.url,
                         'http://localhost' + reverse('recipe_show', kwargs={'slug': recipe.slug}))
        ...
        resp = self.app.get(reverse('recipe_edit', kwargs={'user': 'admin', 'slug': 'bad-recipe'}),
